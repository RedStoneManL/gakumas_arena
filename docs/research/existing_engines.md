...
- 公式：`function_cal.py` 的 `cal_base/cal_jizhong/cal_juehaotiao` 均 `math.ceil`。reward（`function_reward.py`）：`0.5·log10(score+1) − 0.03·turn + log10(Δscore) + 白嫖奖励…`，终局 `8·log10(总分)`。
